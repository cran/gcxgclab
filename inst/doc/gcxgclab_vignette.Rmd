...
Gamble, Stephanie & Granger, Caroline & Mannion, Joseph. gcxgclab: An R-package for two dimensional gas chromatography preprocessing and analysis. *Anal. Chem.* 2024, 96, 43, 17059–17064, https://doi.org/10.1021/acs.analchem.4c03126

## Example

Provided here is an example of the work flow to fully process and analyze an example GCxGC sample. The data used in this example is available for download here: https://doi.org/10.6084/m9.figshare.26060830.v1
...
For experienced users, batch extraction and preprocessing functions are available. Note that the preprocessing() and batch_preprocessing perform the data extraction; the input should be the raw data file name or folder path.

``` r
preprocessed_sample <- preprocess(filename)
folder <- system.file('extdata',package='gcxgclab')
batch_raw_data <- batch_extract(folder)
preprocessed_batch <- batch_preprocess(folder)
```
...
The area of the entire TIC can also be calculated, flattened in 1 time dimension. Optional start and dending times can be specified if not the entire dataset.

``` r
TIC_integrate(frame)
```

The area for this sample is:
